...
def main() -> None:
    ...
    try:
        ...
        target_ids = {str(row[0]) for row in con.execute("SELECT DISTINCT target_id FROM gbdt_prediction_runs")}
        for spec in TARGET_SPECS:
            if spec.target_id not in target_ids:
                raise RuntimeError(f"No gbdt runs for target_id={spec.target_id}")
        runs = int(con.execute("SELECT COUNT(*) FROM gbdt_prediction_runs").fetchone()[0])
        outputs = int(con.execute("SELECT COUNT(*) FROM gbdt_prediction_outputs").fetchone()[0])
        print("prediction gbdt validation ok")
        print(f"runs={runs}")
